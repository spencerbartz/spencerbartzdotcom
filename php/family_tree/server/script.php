...
$familyname =  $_GET["dbname"] . '/';

 move_uploaded_file($_FILES['Filedata']['tmp_name'], strtolower('../uploads/' . $familyname . $_FILES['Filedata']['name']));
...
if ($error) {
	...
	$return['hash'] = md5_file(strtolower('../uploads/' . $familyname . $_FILES['Filedata']['name']));

	// ... and if available, we get image data
	$info = @getimagesize(strtolower('../uploads/' . $familyname . $_FILES['Filedata']['name']));
